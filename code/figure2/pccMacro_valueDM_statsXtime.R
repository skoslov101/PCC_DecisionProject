...
baseDir='/Volumes/SRKephys/FosterLab/ephys/Manuscript/CodeRepo/data/valueDM'
...
regWofLoc=summaryBy(psc~time*elecID*blockVal*locName,data=pccDat,FUN=c(mean))
regWofLoc2=summaryBy(psc.mean~time*elecID*locName,data=regWofLoc,FUN=c(mean))
names(regWofLoc2)=c('time','elecID','locName','psc')
str(regWofLoc2)

choicePeriodLoc=regWofLoc2[(regWofLoc2$time>1700) & (regWofLoc2$time<2200),]
...
regWofMain1=summaryBy(psc~time*elecID*blockVal*dendClust,data=pccDat,FUN=c(mean))
str(regWofMain1)
...
bothRiskyPlotH1
riskyH1name=sprintf('%s/macroPCC_wof_choice_smoothPSC_xRisky_xClust.pdf',outDir)
...
#Load the response-locked data
pccDat=read.csv(sprintf('%s/pccMacro_valueBasedDM_responseLocked.csv',baseDir),header=T)
head(pccDat)
...
respPeriodLoc=regWofLoc2[(regWofLoc2$time>1500) & (regWofLoc2$time<2000),]
head(respPeriodLoc)
...
lmResp0=lmer(psc.mean~clustVal+(1|Subject),data=regWofMain3b)
summary(lmResp0)
confint(lmResp0,method="Wald")

rd0=regWofMain3b[regWofMain3b$clustVal=='1',]
rd0lm=lmer(psc.mean~1+(1|Subject),data=rd0)
summary(rd0lm) #
confint(rd0lm,method='Wald')

rv0=regWofMain3b[regWofMain3b$clustVal=='2',]
rv0lm=lmer(psc.mean~1+(1|Subject),data=rv0)
summary(rv0lm)
confint(rv0lm,method='Wald')
...
lmResp0=lmer(diff~clustVal+(1|Subject),data=respSum2all)
summary(lmResp0)
confint(lmResp0,method="Wald")

rd0r=respSum2all[respSum2all$clustVal=='1',]
rd0rLM=lmer(diff~1+(1|Subject),data=rd0r)
summary(rd0rLM)
confint(rd0rLM,method='Wald')

rv0r=respSum2all[respSum2all$clustVal=='2',]
rv0rLM=lmer(diff~1+(1|Subject),data=rv0r)
summary(rv0rLM)
confint(rv0rLM,method='Wald')
...
bothRiskyPlotH1b
riskyH1name=sprintf('%s/macroPCC_wof_delay_smoothPSC_xRisky_xClust.pdf',outDir)
...
#Load the response-locked data
pccDat=read.csv(sprintf('%s/pccMacro_valueBasedDM_feedbackLocked.csv',baseDir),header=T)
head(pccDat)
...
feedPeriodLoc=regWofLoc2[(regWofLoc2$time>1500) & (regWofLoc2$time<2000),]
head(feedPeriodLoc)
...
lmResp0=lmer(diff~clustVal+(1|Subject),data=feedSum2all)
summary(lmResp0)
confint(lmResp0,method="Wald")
...
bothRiskyPlotH1b=ggplot() + theme_bw() + geom_line(data=smoothRiskHb,aes(x=time,y=smoothDat,color=clustVal,linetype=wasRisky)) + geom_ribbon(data=smoothRiskHb,aes(x=time,ymin=smoothDat-smoothSE,ymax=smoothDat+smoothSE,fill=clustVal,linetype=wasRisky),alpha=.3) + geom_vline(xintercept=1500,linetype='solid',color='black') + scale_y_continuous(limits=c(-12,22),breaks=c(-10,-5,0,5,10,15,20)) + theme(panel.grid.minor = element_blank())
bothRiskyPlotH1b
riskyH1name=sprintf('%s/macroPCC_wof_feedback_smoothPSC_xRisky_xClust.pdf',outDir)
ggsave(riskyH1name,height=7,width=10,bothRiskyPlotH1b)
...
allDiffPlot
allDiffName=sprintf('%s/macroPCC_wof_riskModSummary_allTimes_clust12.pdf',outDir)
