...
microDat=read.csv(sprintf('%s/pccMicro_valueBasedDM_concatTimeseries.csv',baseDir),header=TRUE)
...
disMatName1=sprintf('%s/pccMicro_valueDM_dissMatrix.pdf',outDir)
...
# Dendrogram
clusters <- cutree(hc, k =3)  # for X clusters
...
dend_colored <- color_branches(dend, k = 3,col=my_colors)
dend_colored <- color_labels(dend_colored, k = 3,col = my_colors)
...
dendName=sprintf('%s/pccMicro_valueDM_hcClust.pdf',outDir)
